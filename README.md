...
Índice de endpoints
Todas las rutas parten de la base https://{llave}.connect.axoft.com/api/eCommerce. Todos los endpoints requieren los headers ApiAuthorization y Company.

Método	Ruta	Descripción
POST	/Dummy	Verificación de credenciales (prueba de autenticación).
POST	/Order	Recibe una orden de la tienda para generar el pedido en Tango. Requiere ?Cuenta=.
POST	/Order/Batch	Recibe órdenes en lote (hasta 25) para generar los pedidos en Tango. Requiere ?Cuenta=.
GET	/ArtPorSaldoStock	Artículos filtrados por saldo de stock.
GET	/CounterFoil	Talonarios.
GET	/Currency	Monedas.
GET	/CurrencyExchangeRate	Cotizaciones de moneda extranjera.
GET	/Customer	Clientes.
GET	/CustomersFolder	Clasificador de clientes: clientes en carpetas.
GET	/CustomersFolderClassifier	Clasificador de clientes: carpetas.
GET	/CustomersRelation	Clasificador de clientes: relaciones.
GET	/DiscountByCustomer	Descuentos por cliente.
GET	/Invoices	Relación entre cada orden y el comprobante electrónico de su pedido facturado (con enlace al PDF).
GET	/Measure	Unidades de medida.
GET	/Order	Obtener información de las órdenes recibidas.
GET	/Price	Precios de artículos.
GET	/PriceByCustomer	Precios por cliente.
GET	/PriceList	Listas de precios.
GET	/Product	Artículos (composición, comentarios y escalas).
GET	/ProductsFolder	Clasificador de artículos: artículos en carpetas.
GET	/ProductsFolderClassifier	Clasificador de artículos: carpetas.
GET	/ProductsRelation	Clasificador de artículos: relaciones.
GET	/Publications	Publicaciones (relación artículo de la tienda ↔ artículo de Tango).
GET	/SaleCondition	Condiciones de venta.
GET	/Scale	Escalas.
GET	/Seller	Vendedores.
GET	/Stock	Saldos de stock por artículo/depósito/sucursal.
GET	/StockGroupByProduct	Saldos de stock agrupados por artículo.
GET	/Store	Sucursales.
GET	/Transport	Transportes.
GET	/ValueScale	Valores de escala.
GET	/Warehouse	Depósitos (incluye depósitos de stock centralizado).
<a name="instalacion">
...
Una Cuenta de Integración API creada. Su nombre es el valor que se envía en el parámetro ?Cuenta= de los POST (y opcionalmente como filtro en los GET Order, Invoices y Publications).
...
Las fórmulas de validación de Total y PaidTotal comparan los valores redondeados a 2 decimales.
El criterio de redondeo funciona igual que el redondeo tradicional, salvo cuando el valor queda exactamente a mitad de camino (la parte descartada es exactamente 5): en ese caso, en lugar de redondear siempre hacia arriba, el segundo decimal queda en el valor par más cercano. Ejemplos: 10.126 → 10.13 y 10.124 → 10.12 (igual que el redondeo tradicional); 10.125 → 10.12 (el 2 ya es par) y 10.135 → 10.14 (el 3 sube al par 4). - A la API se accede a través de Tango Connect: la URL base tiene el formato https://{llave}.connect.axoft.com/api/eCommerce, donde {llave} es el número de llave del sistema Tango reemplazando la barra / por un guion - (por ejemplo, la llave 000000/000 corresponde a https://000000-000.connect.axoft.com/api/eCommerce).
...
Para consumir la API sólo necesita las credenciales de acceso. Lo que debe estar configurado en Tango se lista en Requisitos previos.
...
Company: 1
Ambos headers son obligatorios en todos los endpoints (incluidos los GET). En ausencia de cualquiera de los dos, o en caso de que su valor sea vacío, obtendrá como respuesta un código de error 401.

<a name="dummy">

Verificar la conexión (Dummy)
Para validar las credenciales, realice un POST (sin body) al recurso Dummy:
...
Data	Lista con los registros del recurso. Presente en toda respuesta con succeeded: true (un GET sin resultados devuelve []). En las respuestas de error con succeeded: false la clave puede no emitirse; chequee succeeded antes de leerla.
PagingError	Detalle de error de paginación; null cuando la paginación es válida.
OrderError	Detalle de error específico de órdenes (ver GET Order); null en el resto de los casos.
...
Importante — fin de la paginación: la última página se detecta con Paging.MoreData: false (o Data vacío). Si migra desde la API de Tango Tiendas, el terminador del bucle de sincronización cambió: consúltelo en Migración.
...
Datos inválidos / error de binding (GET o POST)	400	Cuerpo JSON con el detalle por campo en la propiedad errors (ver ejemplo). Incluye un tipo inválido en la query de un GET, p. ej. PageSize=abc (no numérico)
...
Advertencia — error de tipeo en el prefijo: el 404 JSON aplica sólo a rutas bajo el prefijo exacto Api/eCommerce.
...
El parámetro Cuenta es obligatorio y corresponde al nombre de la Cuenta de Integración API.

La orden se envía en el body en formato JSON. Consulte:
...
La respuesta usa la forma Status/Message/Data/isOk (la misma que devuelve Dummy), más los campos succeeded y OrderError. En caso de éxito (Status: 0, isOk: true; Data viene siempre y es null en la orden individual):
...
Ante un error de negocio, la respuesta indica Status: 1, isOk: false, succeeded: false y el texto real de la validación en Message. Si el nombre enviado en Cuenta no corresponde a ninguna Cuenta de Integración API, la respuesta usa esa misma forma con Message: "<cuenta> is not registered.". Ante un error de binding (por ejemplo, falta Customer.Email), la respuesta es 400.

<a name="notificaciones">

Notificaciones (webhooks)
Volver al índice

Las notificaciones se habilitan durante la puesta en marcha de la Cuenta de Integración API, indicando una URL de notificaciones y los eventos a recibir. El receptor debe exponer un endpoint POST de una web API accesible por HTTPS (TLS 1.2 o superior).
...
Importante — sensibilidad a mayúsculas: los nombres de las propiedades (Topic, Resource, Message) y los valores de Topic son case sensitive; respételos tal como figuran.
...
El body contiene una lista de órdenes (con el mismo formato que el POST Order):
...
Con los headers ApiAuthorization (Token de desarrollador) y Company (IdEmpresa) en todas las llamadas. Para los POST de órdenes, además el parámetro Cuenta en la URL. Ver Obtener credenciales.
...
<details> <summary>Ejemplo: DUM informando unidad de Ventas</summary>
...
En este caso se informa la unidad de Ventas (SelectMeasureUnit: "V"), por lo que UnitPrice corresponde al precio de venta; al generar el pedido se expresará en la unidad de stock 1. </details>

<details> <summary>Ejemplo: DUM informando unidad de Stock 1</summary>
...
En este caso se informa la unidad de Stock 1 (SelectMeasureUnit: "P"), por lo que UnitPrice corresponde al precio por unidad de stock 1 (kilogramo). </details>

Condición de venta, transporte y pagos
Condición de venta (SaleConditionCode): si es distinta de Contado, es posible que al valor de la factura se le apliquen cargos propios de la condición (por ejemplo, intereses a 30/60/90 días).
Transporte: si la condición de venta es Contado, se valida que el transporte informado no tenga recargo (SurchargePercentage = 0).
...
WarehouseCode	No	Alfanumérico (≤10)	Código de depósito.
SellerCode	No	Alfanumérico (≤10)	Código de vendedor.
TransportCode	No	Alfanumérico (≤10)	Código de transporte.
SaleConditionCode	No	Entero (0–99)	Condición de venta.
PriceListNumber	No	Entero	Número de lista de precios.
InvoiceCounterfoil	No	Entero (0–9999)	Talonario de facturación.
OrderCounterfoil	No	Entero (0–9999)	Talonario de pedido.
AgreedWithSeller	No	Lógico	Indica si el pago se acuerda con el vendedor.
ValidateTotalWithItems	No	Lógico	Si es true, valida Total según la fórmula.
ValidateTotalWithPaidTotal	No	Lógico	Si es true, valida además que PaidTotal no exceda Total (ver fórmulas).
...
<a name="formulas"> Fórmulas de validación
...
DocumentType	Sí	Alfanumérico	Código de tipo de documento. Ver Tipo de Documento.
DocumentNumber	No	Alfanumérico	Número de documento sin símbolos.
IVACategoryCode	Sí	Alfanumérico	Categoría de IVA. Ver Condición Fiscal.
User	Sí	Alfanumérico (≤200)	Usuario de la tienda.
Email	Sí	Alfanumérico (≤255)	Correo electrónico.
ProvinceCode	Sí	Alfanumérico (≤4)	Código AFIP de la provincia. Ver Provincias.
...
NumberListPrice	No	Entero	Número de lista de precios del cliente.
PayInternalTax	No	Lógico	Indica si se liquidan impuestos internos al comprador.
<a name="clientehabitual"> Cómo se relaciona con el cliente habitual

Si en los parámetros de eCommerce está activada la opción Busca cliente habitual, la orden se asocia a un cliente habitual de Tango según el primer criterio que arroje coincidencia, en este orden:

ABM Clientes — solapa principal: código de cliente (Code), luego tipo y número de documento, luego correo electrónico.
ABM Clientes — solapa contactos (si la solapa principal no arrojó coincidencias): código de cliente, tipo y número de documento, correo electrónico y, por último, usuario de la tienda (User).
...
SKUCode	No	Alfanumérico (≤40)	Código de artículo, sinónimo o código de barras de Tango. Establece/actualiza la relación con el artículo de Tango.
VariantCode	No	Alfanumérico (≤200)	Código de la combinación (escala).
...
SelectMeasureUnit	No	Alfanumérico (1)	Unidad de medida seleccionada: V, P o S (por defecto V). Ver DUM.
MeasureCode	No	Alfanumérico	Código de medida con el que se genera el pedido.
WarehouseCode	No	Alfanumérico (≤10)	Código de depósito del renglón. Si se informa, el renglón del pedido se genera con ese depósito.
...
PaymentMethod	Sí	Alfanumérico (≤3)	Código de forma de cobro. Ver Formas de Pago.
PaymentTotal	Sí	Numérico (≥0)	Total del pago. El servidor no lo exige a nivel de binding: si se omite se asume 0, y su valor debe cuadrar con PaidTotal y la suma de pagos.
...
<details> <summary>Tipo de Documento</summary>
...
Son los tipos de documento habilitados para el ingreso de comprobantes en Tango (IVA_TIPO_DOCUMENTO.HABILITADO_PARA_INGRESO = 'S'). Un código no habilitado (por ejemplo 89 - LE) se rechaza con Incorrect value - Field: DocumentType. </details>

<a name="provincias">

<details> <summary>Provincias</summary>
...
<details> <summary>Condición Fiscal</summary>
...
<details> <summary>Formas de Pago</summary>
...
<details> <summary>Orden con condición de venta Contado</summary>
...
<details> <summary>Orden con condición de venta Cuenta corriente</summary>
...
<details> <summary>Orden con código de dirección de entrega</summary>
...
En este caso se informa Customer.Code para enlazar la orden con el cliente habitual de Tango (ver cómo se relaciona con el cliente habitual) y PriceListNumber; el desglose impositivo del pedido se define según la configuración de esa lista de precios en Tango. </details>
...
Los recursos GET permiten consultar datos de Tango Gestión o Tango Punto de Venta, devolviendo el resultado paginado en el formato de respuesta descripto arriba. Todos se consumen sobre la misma base; por ejemplo:
...
Nota sobre stock centralizado: si la empresa centraliza stock de varias sucursales y desea informar saldos centralizados, envíe Centraliza=true en Stock / StockGroupByProduct. De lo contrario, la consulta devuelve los saldos de la sucursal local (ver Consideraciones).
...
Comprobantes de facturación (Invoices)
Publicaciones (Publications)
Sucursales (Store)
Depósitos (Warehouse)
Unidades de medida (Measure)
Escalas (Scale) · Valores de escala (ValueScale)
Artículos (Product) · Artículos por saldo de stock (ArtPorSaldoStock)
Saldos de stock (Stock) · Saldos agrupados (StockGroupByProduct)
Listas de precios (PriceList) · Precios (Price)
Precios por cliente (PriceByCustomer) · Descuentos por cliente (DiscountByCustomer)
Clientes (Customer) · Vendedores (Seller)
Transportes (Transport) · Condiciones de venta (SaleCondition)
Monedas (Currency) · Cotizaciones (CurrencyExchangeRate)
Talonarios (CounterFoil)
Clasificador de artículos: ProductsFolder · ProductsRelation · ProductsFolderClassifier
Clasificador de clientes: CustomersFolder · CustomersRelation · CustomersFolderClassifier
<a name="recorder">
...
Si se supera el límite de 50 identificadores por consulta, la consulta entera se rechaza sin ejecutarse. La respuesta conserva el envelope paginado — Paging con la paginación pedida, Data: [], PagingError: null — con succeeded: false y el detalle en OrderError: los identificadores enviados en Orders y Message: "The number of orders exceeds the limit of 50". </details>
...
Recuerde: ProvinceCode es el código AFIP de la provincia (ver Provincias).
...
Devuelve los artículos con su composición, comentarios y valores de escala. Cada artículo de la página sale con todos sus comentarios (ProductComments) y renglones de composición (ProductComposition), cualquiera sea la página o los filtros aplicados. Sólo se muestran artículos que en Tango cumplan: perfil de Venta, Compra-Venta o Inhabilitado; tipo Simple, Fórmula o Kit fijo; y que no sean artículos Base.
...
Devuelve artículos (con la misma forma que Product) filtrados por saldo de stock.
...
La respuesta tiene la misma estructura que Product, incluidos ProductComments y ProductComposition completos por artículo.
...
StoreNumber	string	Número de sucursal (valor único; el operador depende de UseEqual). Para consultar varias sucursales a la vez, use StockGroupByProduct.
...
Centraliza	bool	Incluir stock centralizado.
groupByProduct	bool	Si es true, la consulta se resuelve como StockGroupByProduct (saldo agrupado por artículo). Por defecto false.
stockGroupByProduct	bool	Alias de groupByProduct: si es true, la consulta se resuelve como StockGroupByProduct. Por defecto false.
<details> <summary>Respuesta</summary>
...
<a name="migplataforma"> Plataforma y autenticación
...
Selección de tienda en POST	embebida en el token	parámetro ?Cuenta=
...
Códigos de estado: el resultado de la operación se consulta en el campo succeeded (ver Códigos de estado HTTP). Cambios puntuales respecto de Tiendas:
...
GET ArtPorSaldoStock y GET StockGroupByProduct son rutas nuevas de eCommerce; la capacidad ya existía en Tiendas (respectivamente, POST DataBy/ArtPorSaldoStock y Stock con GroupByProduct=true).
POST Dummy no es un recurso nuevo: ya existía en Tiendas y sólo cambia el mensaje de la prueba de autenticación (ver la tabla de plataforma más arriba).
Parámetros
...
Los parámetros sort y validatePageSize de Tiendas no están disponibles en eCommerce; lastUpdate se conserva sólo en CurrencyExchangeRate (ver el recurso).
<a name="consideraciones">

Consideraciones
Volver al índice

Consulte siempre succeeded (y Message / OrderError para el detalle) para determinar el resultado de cada operación. Ver Códigos de estado HTTP.
Stock centralizado: tenga en cuenta que si no envía Centraliza=true en Stock / StockGroupByProduct, la consulta devuelve los saldos de la sucursal local. Si la empresa centraliza el stock de varias sucursales y desea obtener los saldos centralizados, envíe Centraliza=true.
Los límites operativos están documentados en la sección de cada recurso: tamaño y fin de página, órdenes por lote, identificadores por consulta en GET Order.
